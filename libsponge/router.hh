...
class RouteEntry {
  public:
    ...
    uint32_t _subnet_mask;
    RouteEntry(uint32_t rp, uint8_t pl, std::optional<Address> nh, size_t in, uint32_t sm): 
        _route_prefix(rp), _prefix_length(pl), _next_hop(nh), 
        _interface_num(in), _subnet_mask(sm){}
};
